...
A profiling suite that measures the GPU kernels behind LLM inference (attention and KV-cache decode) on an NVIDIA Jetson Orin Nano, a 15 W edge SoC, to find what bottlenecks low-cost local LLM-agent deployment. Profiled with Nsight Systems and PyTorch on JetPack.

## Key results (Jetson Orin Nano, fp16)

| Result | Value | Source |
|---|---|---|
| Fused **SDPA / FlashAttention** vs naive matmul/softmax/matmul attention (S=1024) | **5.55x lower p50 latency** | `results/raw_csv/attn_compare.csv` |
| Single-stream **KV-cache decode** throughput at 1K context | **~15.4K tokens/sec** (64.9 us/token) | `results/raw_csv/decode_kv_sweep.csv` |
| Decode latency scaling (context 256 to 8192) | **63.1 to 398.1 us/token** | `results/raw_csv/decode_kv_sweep.csv` |
| Peak fp16 GEMM throughput | **7.46 TFLOP/s** (1024 cubed, batch 16) | `results/raw_csv/bench.csv` |

The decode sweep shows the expected memory-bound behavior: throughput is flat (about 15.6 to 15.9K tok/s) up to roughly 1K context, then falls off as the KV cache grows (about 8.7K at 2K, 2.5K at 8K). Nsight traces show the matching per-token kernel-launch pattern, one `cudaLaunchKernel` per decode step, which is the classic small-batch decode overhead on constrained hardware.

## Plots

![SDPA vs naive attention latency vs sequence length](results/plots/attn_latency_vs_seq_compare.png)

![KV-cache decode us/token vs context length](results/plots/decode_us_per_token_vs_kv.png)
...
- **KV-cache decode** (`src/kv_decode_bench.py`, `src/kv_decode_microbatch.py`): single-query decode step swept over KV-cache length, reported as us/token.
- **GEMM** (`src/bench.py`): batched fp16 GEMM throughput sweep with full device and JetPack metadata captured per row.
...
- Tokens/sec is derived from single-stream decode latency (1 / us-per-token), not a batched serving throughput.
- The kernel-fusion finding is the SDPA-vs-naive gap plus the Nsight launch-pattern trace; it characterizes the kernels rather than optimizing a deployed model.
